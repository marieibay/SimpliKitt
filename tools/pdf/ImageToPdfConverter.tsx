...
const ImageToPdfConverter: React.FC = () => {
  ...
  const [libraryReady, setLibraryReady] = useState(!!window.jspdf);
  ...
  return (
    <div className="space-y-6">
      {!outputPdfUrl && (
        <>
            <div {...getRootProps()} className={`p-8 border-2 border-dashed rounded-lg text-center transition-colors ${!libraryReady || !!error ? 'border-gray-300 bg-gray-100 cursor-not-allowed' : isDragActive ? 'border-blue-500 bg-blue-50 cursor-pointer' : 'border-gray-300 hover:border-gray-400 cursor-pointer'}`}>
                <input {...getInputProps()} />
                ...
                <p className="text-sm text-gray-500">{libraryReady ? 'Supports JPG, PNG, WEBP' : 'Initializing converter...'}</p>
            </div>
