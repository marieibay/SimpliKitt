...
const ImageHueShifter: React.FC = () => {
    ...
    }, [image, hue, applyFilter]);

    const handleDownload = () => {
        if (!resultUrl) return;
        trackEvent('image_filter_applied', { filter: 'hue-shift', value: hue });
        const link = document.createElement('a');
        link.href = resultUrl;
        link.download = `hue-shifted-image.png`;
        link.click();
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                <label className="block text-sm font-medium">Hue Shift: {hue}°</label>
                <input type="range" min="-180" max="180" value={hue} onChange={e => setHue(parseInt(e.target.value, 10))} className="w-full" />
            </div>
...
export default ImageHueShifter;
