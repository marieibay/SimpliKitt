...
const BulkPngToJpgConverter: React.FC = () => {
    ...
    const handleConvertAndDownload = async () => {
        ...
        for (const file of files) {
            ...
            if(ctx) {
                ctx.fillStyle = "#FFFFFF"; // Fill background for transparency
        ...
        link.download = "converted-to-jpg.zip";
        link.click();
        
        trackEvent('bulk_images_converted', { count: files.length, from: 'png', to: 'jpg' });
        trackGtagEvent('tool_used', {
            event_category: 'Image Tools',
            event_label: 'Bulk PNG to JPG Converter',
            tool_name: 'bulk-png-to-jpg-converter',
            is_download: true,
            ...
            {files.length > 0 && (
                <div className="space-y-4">
                    ...
                    <button onClick={handleConvertAndDownload} disabled={isProcessing} className="w-full px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 disabled:bg-blue-300">
                        {isProcessing ? `Converting ${files.length} images...` : `Convert ${files.length} PNGs to JPG & Download`}
                    </button>
                </div>
            )}
        </div>
